...
privacysociety_settings_enabled="$(settings get global persist.privacysociety.settings)"
if $privacysociety_settings_enabled == true; then
    $(echo "privacysociety.settings exist" > /dev/kmsg)
    return
fi

$(echo "writing privacysociety.settings" > /dev/kmsg)
...
settings put global persist.privacysociety.settings.gestural false
